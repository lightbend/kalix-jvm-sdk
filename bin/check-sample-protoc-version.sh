...
AKKA_GRPC_PROTOC_FULL=$(sbt --client "print coreSdk/protocVersion" 1>/tmp/kalix-version.txt 2>/dev/null)
# the tail distance is different locally and on CI
AKKA_GRPC_PROTOC=$(cat /tmp/kalix-version.txt | tail -n 3 | head -n 1 | xargs)
