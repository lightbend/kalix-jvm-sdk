...
class Counter(context: ValueEntityContext) extends AbstractCounter {
  ...
  override def increase(currentState: CounterState, increaseValue: counter_api.IncreaseValue): ValueEntity.Effect[Empty] =
    effects.error("The command handler for `Increase` is not implemented, yet")

  override def decrease(currentState: CounterState, decreaseValue: counter_api.DecreaseValue): ValueEntity.Effect[Empty] =
