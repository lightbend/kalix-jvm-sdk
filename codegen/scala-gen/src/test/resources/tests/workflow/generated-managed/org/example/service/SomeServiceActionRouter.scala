...
class SomeServiceActionRouter(action: SomeServiceAction) extends ActionRouter[SomeServiceAction](action) {

  override def handleUnary(commandName: String, message: MessageEnvelope[Any]):  Action.Effect[_] = {
    commandName match {
      case "simpleMethod" =>
        action.simpleMethod(message.payload.asInstanceOf[SomeRequest])
