    ...
    services.flatMap { case service: ModelBuilder.Service =>
      ...
      if (!implSourcePath.toFile.exists()) {
        ...
        val testSourceFiles = if (!testSourcePath.toFile.exists()) {
          ...
          val _ = Files.write(
            testSourcePath,
            testSource(service, packageName, implClassName, testClassName).layout.getBytes(
              Charsets.UTF_8
            )
          )
          List(testSourcePath)
        ...
        val _ = Files.write(
          implSourcePath,
          source(service, packageName, implClassName, interfaceClassName).layout.getBytes(
            Charsets.UTF_8
          )
        )
      ...
      if (services.nonEmpty) {
        ...
        if (!mainClassPath.toFile.exists()) {
          ...
          val _ = Files.write(
            mainClassPath,
            mainSource(mainClassPackageName, mainClassName, services).layout.getBytes(
              Charsets.UTF_8
            )
          )
          List(mainClassPath)
