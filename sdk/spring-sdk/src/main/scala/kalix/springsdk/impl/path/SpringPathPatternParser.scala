...
/**
 * I do wish I didn't have to write this myself, but unfortunately, the
 * [[org.springframework.web.util.pattern.PathPattern]] parsed by
 * [[org.springframework.web.util.pattern.PathPatternParser]] only offers methods for actually matching paths, there's
 * no way to inspect the structure of the parsed path, hence we can't use that to translate a parsed Spring template
 * path to a Google gRPC HTTP transcoding path template.
 *
 * // todo test me
 */
