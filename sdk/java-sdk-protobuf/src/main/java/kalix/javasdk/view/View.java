 ...
 *
 * <p>When implementing a View, you define what will be its internal state (your view model) and how
 * you want to query it. The Query string defines which fields will be indexed and how the query
 * will be executed.
 *
 * <p>The query is executed by the Kalix when a request is made to the View.
 *
 * <p>Views are updated in response to Event Sourced Entity events, Value Entity state changes or
 * messages from a Topic.
 *
 * <p>Each incoming change is handled by a command handler. Command handlers are methods returning
 * an {@link UpdateEffect}. The command handler is responsible for updating the View state.
 *
...
public abstract class View<S> {
  ...
  /**
   * An UpdateEffect is a description of what Kalix needs to do after the command is handled. You
   * can think of it as a set of instructions you are passing to Kalix. Kalix will process the
