...
public class ProtoStepBuilder {
   ...
   *
   * <p>The {@link Function} passed to this method should return a {@link DeferredCall}. The {@link
   * DeferredCall} is then executed by Kalix and its result, if successful, is made available to
   * this workflow via the {@code andThen} method. In the {@code andThen} method, we can use the
   * result to update the workflow state and transition to the next step.
   *
   ...
   */
  @ApiMayChange
  public <Input extends GeneratedMessageV3, DefCallInput, DefCallOutput extends GeneratedMessageV3>
      CallStepBuilder<Input, DefCallInput, DefCallOutput> call(
          Class<Input> callInputClass,
          Function<Input, DeferredCall<DefCallInput, DefCallOutput>> callFactory) {
   ...
   * Build a step action with a call to an existing Kalix component via {@link DeferredCall}.
   *
   * <p>The {@link Supplier} function passed to this method should return a {@link DeferredCall}.
   * The {@link DeferredCall} is then executed by Kalix and its result, if successful, is made
   * available to this workflow via the {@code andThen} method. In the {@code andThen} method, we
   * can use the result to update the workflow state and transition to the next step.
   *
   * <p>On failure, the step will be retried according to the default retry strategy or the one
   * defined in the step configuration.
   *
   ...
   */
  @ApiMayChange
  public <DefCallInput, DefCallOutput extends GeneratedMessageV3>
      CallStepBuilder<Void, DefCallInput, DefCallOutput> call(
   ...
   */
  @ApiMayChange
  public <Input extends GeneratedMessageV3, Output extends GeneratedMessageV3>
      AsyncCallStepBuilder<Input, Output> asyncCall(
   ...
   */
  @ApiMayChange
  public <Output extends GeneratedMessageV3> AsyncCallStepBuilder<Void, Output> asyncCall(
  ...
  public static class CallStepBuilder<Input, DefCallInput, DefCallOutput> {
     ...
     */
    @ApiMayChange
    public Workflow.CallStep<Input, DefCallInput, DefCallOutput, ?> andThen(
        Class<DefCallOutput> transitionInputClass,
        Function<DefCallOutput, Workflow.Effect.TransitionalEffect<Void>> transitionFunc) {
