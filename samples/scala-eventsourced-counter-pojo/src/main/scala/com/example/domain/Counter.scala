...
class Counter(context: EventSourcedEntityContext) extends AbstractCounter {
  ...
  override def emptyState: CounterState = CounterState(0)
  ...
  override def decrease(
    ...
    if (decreaseValue.value < 0) {
      effects.error("Value must be a zero or a positive number")
    } else if (currentState.value - decreaseValue.value < 0) {
      // we want to force a crash to be able to reload 
      throw new IllegalArgumentException("Decrease value is too large, counter would become negative")
    } else {
      effects
