...
```shell
curl -XPOST -H "Content-Type: application/json" localhost:9000/com.example.fibonacci.FibonacciAction/NextNumber -d '
{"value": 5 }'
```
...
```shell
grpcurl -plaintext -d '{"value": 5 }' localhost:9000 com.example.fibonacci.FibonacciAction/NextNumber
```
