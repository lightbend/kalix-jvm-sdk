...
///HOW do I really call the the effects? 
  ...
  private def interpretEffects[R](effect: EventSourcedEntity.Effect[R], command: String): EventSourcedResult[R] = {
  ...
  def increase(command: example.IncreaseValue): EventSourcedResult[Empty] =
    interpretEffects(entity.increase(_state, command), command.toString)

  def increaseWithSideEffect(command: example.IncreaseValue): EventSourcedResult[Empty] =
    interpretEffects(entity.increaseWithSideEffect(_state, command), command.toString)

  def decrease(command: example.DecreaseValue): EventSourcedResult[Empty] =
    interpretEffects(entity.decrease(_state, command), command.toString)

  def reset(command: example.ResetValue): EventSourcedResult[Empty] =
    interpretEffects(entity.reset(_state, command), command.toString)

  def getCurrentCounter(command: example.GetCounter): EventSourcedResult[example.CurrentCounter] =
    interpretEffects(entity.getCurrentCounter(_state, command), command.toString)
