...
Note that if link:https://docs.kalix.io/operations/observability-exports.html#_activating_tracing_beta[tracing] is not enabled in your service this `Optional` will be empty. Otherwise you can use the `tracerOpt.get()` to retreive the tracer. 

To create a link:https://opentelemetry.io/docs/languages/java/instrumentation/#create-spans[span] and close it over an asynchronous call, you can do the following: 
...
----
<1> setting the action's TraceContext as the parent of this span. Linking the action's trace with this span.
<2> creating and starting the span.
<3> adding some attribute.
<4> make the span the current span.
<5> adding some attribute.
<6> closing the span.
<7> setting the status of the span as error.
