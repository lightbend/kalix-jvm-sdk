...
An Action's gRPC method can be triggered automatically when some predefined service life-cycle event happens, serving as a custom hook. For such use, the method needs be annotated with a specific flag as shown below and its must be of type `google.protobuf.Empty`.
...
<2> method must receive `google.protobuf.Empty` as input type;
<3> select which event triggers this hook (in this case, this is once instance startup is concluded);
