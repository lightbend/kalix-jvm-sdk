...
If tracing [link to our docs] is not activated the `get()` method will throw an `java.util.NoSuchElementException`. You can of course check first if tracer `isPresent`. But for the sake of simplicity here that is not done.   

To create it and close it over an asynchronous call: 
...
----
<1> setting this action span as the parent of this span.
...
NOTE: Make sure you understood how tracing is enabled [link to that when https://github.com/lightbend/kalix-docs/pull/2000 merged to main]
