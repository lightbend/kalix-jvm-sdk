...
Actions return effects, that is, `Action.Effect`. With this type we can return a reply, an error, forward the call to another component, and attach effects to messages. Here we want only the result of the calculation or an error. Therefore we use `.reply` and `.error`.
