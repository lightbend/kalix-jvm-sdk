...
Actions return effects, that is, `Action.Effect`. We have different types of effects. A reply, an error, forward the call to another component, and effects with side effects. Here we want only the result of the calculation or an error. Therefore we use `.reply` and `.error`.
