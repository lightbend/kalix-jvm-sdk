...
In this first example, we will show how to
implement an Action as a pure stateless function. We will define a `FibonacciAction` that takes a number and returns the
next number in the Fibonacci series.

== Implementing the Action

To implement this action we need the following: 

- Extend our class from `kalix.javasdk.action.Action`. This is generic. No matter what action we want to create we always need to extend from link:{attachmentsdir}/testkit/kalix/javasdk/action/Action.html[`Action` {tab-icon}, window="new"]. The rest is specific to this case
...
- Add the Spring annotations @GetMapping and @PostMapping to provide paths for GET and POST to calculate the Fibonacci of a number. Both functions do the same thing and implementation-wise the funtion exposed with GET calls the function exposed with POST.  
...
An Action may return a stream of integers. To do this you need by marking the return type as `reactor.core.publisher.Flux<Effect<Integer>>`. 
...
TODO: create Main docs and add partial here
...
With the `ActionTestkit` we can call the methods of `FibonacciAction`. Each call we execute over to the test kit returns an `ActionResult` that contains the effect produced by the underlying action method.
