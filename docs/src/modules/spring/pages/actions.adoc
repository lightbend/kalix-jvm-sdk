...
With the `ActionTestkit` we can call the methods of `FibonacciAction`. Each call we pass over to the test kit returns an `ActionResult` that contains the effect produced by the underlying action method.
