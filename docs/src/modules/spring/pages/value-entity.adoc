...
----
<1> Expose the `set` command handler on path `/counter/\{counter_id\}/set` as a PUT endpoint where `id` will be its unique identifier.
...
<4> The method is accessible as a POST endpoint on `/counter/\{counter_id\}/plusone`, where `id` will be its unique identifier.
...
----
<1> Expose the `get` query handler on path `/counter/\{counter_id\}` as a GET endpoint where `id` will be its unique identifier.
