...
As mentioned above, to help us illustrate a Value Entity, we will be implementing a Counter service. For such service, we will want to be able to set the initial counter value but also to increase the counter modifying its state. The state will be a simple `Integer` but we will use a wrapper class `Number` for our API, as shown below:
...
Having a good idea of what we want to build and how we will interact with it, we will need to:
...
----
<1> Since this is an entity, we must annotate it with `@Entity` and indicate which path variable
should be used as `entityKey` as well as how we want to name the `entityType`
<2> The `CounterEntity` class should extend `kalix.javasdk.valueentity.ValueEntity`.
<3> The initial state of each counter is defined as 0.
...
----
<1> Expose the `set` command handler on path `/counter/\{id\}/set` as a PUT endpoint where `id` will be its unique identifier.
...
<4> The method is accessible as a POST endpoint on `/counter/\{id\}/plusone`, where `id` will be its unique identifier.
<5> Its `plusone` increases the counter by adding 1 to the current state.
<6> Finally, using the Effect API, we instruct Kalix to persist the new state, and we build a reply with the wrapper object.

NOTE: The `id` parameter matches the `entityKey` value. Also, for this example, we have opted to always repeat the common route `/counter/\{id\}` for each command but a simpler option could be to use a `@RequestMethod("/counter/\{id\}")` at class level.
...
----
<1> Expose the `get` query handler on path `/counter/\{id\}` as a GET endpoint where `id` will be its unique identifier.
...
<2> A built-in webclient is provided to interact with the components.
