...
The Service will contain only one ValueEntity that will expose the operation to mutate a Customer model. The entity itself exposes service endpoints and eventually encapsulates some basic validation. The incoming commands/request are then applied to model and the entity instructs Kalix, thought the `Effect` API what needs to be done next.
