...
== Authentication
 ...
 With Kalix, you can use your JWT tokens to validate access to your endpoints. Typically, these tokens are generated by an authorization server in response to an authentication for a user/password. 
 Kalix can check for the presence of a JWT token when you invoke your endpoint and include it in your component's metadata. The metadata for a call is available through the context (actionContext, commandContext) of the component. 
 Once your component accesses the token via the metadata - in an entry with the "Authorization" key - you can send it to the authorization server for validation so that your component can respond accordingly. 

=== Bearer token validation

If you want to validate that a bearer token is present on a request, this can be done by annotating you endpoint like:
