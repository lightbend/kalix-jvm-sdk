...
[source,java]
----
include::example$spring-hello-world-action/src/main/java/com/example/Main.java[tag=main]
----
...
.src/main/java/com/example/counter/CounterEntity.java
[source,java]
----
include::example$spring-valueentity-counter/src/main/java/com/example/counter/CounterEntity.java[]
----
...
<4> The method is accessible as a POST endpoint on `/counter/{id}/increase`, where `id` will be its unique identifier.
Note that it matches the `entityKey` value.
<5> Its `increaseBy` method receives a `Number` as input and increases the counter by adding it to the current state.
<6> Finally, using the Effect API, we instruct Kalix to persist the new state, and we build a reply.

The `Number` record in this example is used as input for the `increaseBy` method and as to return the
counter's current state.

.src/main/java/com/example/counter/Number.java
[source,java]
----
include::example$spring-valueentity-counter/src/main/java/com/example/counter/Number.java[]
----
...
xref:spring:dependency-injection.adoc[Dependency injection] page for more details on how Kalix components integrates
