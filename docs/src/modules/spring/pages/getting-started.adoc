...
* A *Maven plugin* to help you in deploy your application to Kalix.
...
For this *Getting started* walk-through, we will implement the ubiquitous Hello World example. We will use a Kalix
Action that will receive data from a REST endpoint and respond back with a "Hello" message.

.src/main/java/com/example/HelloAction.java
[source,java]
----
include::example$spring-hello-world-action/src/main/java/com/example/HelloAction.java[tag=hello]
----
<1> The `HelloAction` class should extend `kalix.javasdk.action.Action`.
<2> It's `hello` method is annotated with a Spring REST annotation for a `GET`.
<3> Finally, we use the Effects API to construct a reply Effect.

[sidebar]
A Kalix component doesn't require any other annotation. They will be registered to Kalix and be exposed automatically.
It suffices to add them on the same package or on a sub-package of you `Main` class. Please, refer to
...
The project also includes a dependency on the *Kalix Spring SDK Testkit*. Please consult the component specific pages to
