...
Spring developers are used to build Spring Data Repositories to persist and retrieve data. However, in Kalix these tasks are fulfilled by the Proxy, not by the Spring container. Therefore, the usage of Spring Data Repositories is not recommended.
