...
When consuming events from a topic deserialization works out of the box. By default Kalix serialize Java objects to JSON when publish to a topic, or when emiting events from an entity, whatever they are value changes or snapshots. Therefore it can deserialize them as JSON when consuming them. 
