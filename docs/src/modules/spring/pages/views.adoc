...
1. You can declare a catch-all method to receive the `sealed interface` and don't update the View.
2. You can add `ignoreUnknown = true` to your `@Subcribe` annotation but only if it is a type level annotation. 
If an event type can't be handled and there is no `ignoreUnknown = true` at type level, the View will fail. Views are designed to restart, but since the handler is missing, it will fail again.
