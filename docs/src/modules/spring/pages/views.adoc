...
Adding a view ID to your View allows you to refactor the name of the class later on without the risk of losing the history of events. If you don't define a view ID the class name becomes its ID. Therefore, if you change the name of your class afterwards Kalix will not recognize this new name as the same view and will create a brand new view. This is resource consuming for a view from an entity because it will reprocess all the event of that entity to rebuild the view. While for a view built from a topic you will loose all the previous events because these views only process events from current time forwards.
...
WARNING: Views from topics cannot be rebuilt from the source messages, because it's not possible to consume all events from the topic again. The new View is built from new messages published to the topic.
...
. Define the new View, and keep the old View intact. 
. Deploy the new View, and let it rebuild. Verify that the new query works as expected. The old View can still be used.
. Remove the old View but keep its `@ViewId` and `@GetMapping` path, and use them in the new View.
