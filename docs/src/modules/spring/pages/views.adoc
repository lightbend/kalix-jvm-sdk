...
IMPORTANT: It's highly recommended to add a `@TypeName` to your persisted events. Kalix needs to identify each event in order to deliver them to the right event handlers. If no logical type name is specified, Kalix uses the non-qualified class name. However, once a class name is persisted in Kalix, you won't be able to rename your class, as Kalix won't be able to recognize previously persisted events. Therefore, we recommend all event classes to use a logical type name.
