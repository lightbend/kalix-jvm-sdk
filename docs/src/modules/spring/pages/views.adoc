...
Often times, you will want to transform the entity model to which the view is subscribing into a different representation. To do that, let's have a look at the example in which we store a summary of the `Customer` used in the previous section instead of the original one:
...
----
include::example$spring-valueentity-customer-registry/src/main/java/com/example/api/CustomerSummaryByName.java[tag=class]
----
...
----
include::example$spring-valueentity-customer-registry/src/main/java/com/example/api/CustomersStreamByName.java[tag=class]
----
...
----
include::example$spring-valueentity-customer-registry/src/main/java/com/example/api/CustomersResponseByName.java[tag=class]
----
<1> View state type is the original `Customer` as shown at the beginning of this section.
<2> Query is mapped to an external route as usual receiving a `customerName` as parameter.
