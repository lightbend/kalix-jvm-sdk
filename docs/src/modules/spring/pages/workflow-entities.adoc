...
Implementing a business transaction that spans multiple services is one of the major distributes system challenges. Fortunately, with Kalix ecosystem, and concepts like https://docs.kalix.io/spring/actions-publishing-subscribing.html[Subscription], it's very easy to build event driven choreography that covers a complex business process. A https://microservices.io/patterns/data/saga.html[Saga pattern] that generalizes this solution was never so easy to implement. For some use cases a different flavor of the Saga pattern, a choreography, might be more suitable. That was the main driver behind a new Kalix component type called a Workflow Entity. From a high level perspective it joins the power of https://docs.kalix.io/concepts/state-model.html[Kalix Entities state model] (durability, consistency guaranties) and https://docs.kalix.io/spring/actions.html[Kalix Actions] ability to call other components and services. This time you can model your business process in a single place that will keep it running or rollback in case of a failure.
...
<2> An endpoint to deposit funds to the wallet.
<2> An endpoint to get current wallet balance.

Now we could focus on the workflow implementation itself. A workflow has state, which can be updated in command handlers and transition methods. During the state modeling we might consider the data required for validation, running the steps, collecting the information from steps or tracking the workflow progress.
...
<5> With the `transition` method, we inform that the input for the first step is a `Withdraw` object and the name of the step is `"withdraw"`.
...
A full transfer workflow source code is available https://github.com/lightbend/kalix-jvm-sdk/tree/main/samples/spring-transfer-workflow[here]. Follow the Readme file to run and test it.