...
<2> Annotate such class with `@EntityKey`  and pass the name of the key that will be used as the
...
include::partial$entities-key.adoc[tag=intro]

[source,java]
.src/main/java/com/example/shoppingcart/ShoppingCartEntity.java
----
include::example$spring-eventsourced-shopping-cart/src/main/java/com/example/shoppingcart/ShoppingCartEntity.java[tag=generateId]
----
<1> Annotate the command handler with `@GenerateEntityKey`.
<2> Access the generated id and reply.

include::partial$entities-key.adoc[tag=details]
