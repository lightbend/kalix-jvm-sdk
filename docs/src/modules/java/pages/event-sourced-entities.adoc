...
<9> retrieving the state of the entity after all the calls to `addItem`.
+
[.group-scala]#By default the integration and unit test are both invoked by `sbt test`. To invoke only the unit tests run `sbt -DonlyUnitTest test` or set up that value to `true` in the sbt session by `set onlyUnitTest := true` and then run `sbt test`
...
Java::
+
Calling a command handler through the TestKit gives us back an link:{attachmentsdir}/testkit/com/akkaserverless/javasdk/testkit/EventSourcedResult.html[`EventSourcedResult` {tab-icon}, window="new"]. This class has methods that we can use to assert of handling the command, such as:
+
...
Scala::
+
Calling a command handler through the TestKit gives us back an link:{attachmentsdir}/testkit-scala/com/akkaserverless/scalasdk/testkit/EventSourcedResult.html[`EventSourcedResult` {tab-icon}, window="new"]. This class has methods that we can use to assert the result of handling the command, such as:
+
