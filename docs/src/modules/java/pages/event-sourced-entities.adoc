...
<9> retrieving the state of the entity after all the calls to `addItem`.
+
By default the integration and unit test are both invoked by `sbt test`. To invoke only the unit tests run `sbt -DonlyUnitTest test` or set up that paremeter to `true` in the sbt console by `set onlyUnitTest := true` and then run `sbt test`
