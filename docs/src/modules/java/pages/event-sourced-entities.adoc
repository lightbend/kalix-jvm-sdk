...
Once we call a  method on the entity this gives us back a link:{attachmentsdir}/api/com/akkaserverless/javasdk/testkit/EventSourcedResult.html[`EventSourcedResult` {tab-icon}, window="new"]. This class has methods that we can use to assert our expectations, such as:
...
It's worth noting that we have two sources of methods in this unit test. Those from the ShoppingCartTestKit itself and those from the Result. The former gives us the overall state of the entity. Its state and all that it had happened to it over the course of multiple calls. While the latter gives us the effects produced for each singular call.
