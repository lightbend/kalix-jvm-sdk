...
----
<1> Extends the generated `AbstractShoppingCart`, which extends link:{attachmentsdir}/scala-api/com/akkaserverless/scalasdk/eventsourcedentity/EventSourcedEntity.html[`EventSourcedEntity` {tab-icon}, window="new"].
<2> Defines the initial, empty, state that is used before any updates.
...
Scala::
+
Calling an entity method through the TestKit gives us back an link:{attachmentsdir}/testkit-scala/com/akkaserverless/scalasdk/testkit/EventSourcedResult.html[`EventSourcedResult` {tab-icon}, window="new"]. This class has methods that we can use to assert our expectations, such as:
+
* `reply` is the response of the method called in the test
* `allEvents` are the events generated upon the method called in the test. This list can be inspected multiple times. This method is idempotent.
* `nextEvent` is the first of the events generated upon the method called in the test. This event gets consumed once is inspected. This method is not idempotent.
...
Scala::
+
This class is generated by AkkaServerless when the project is compiled and located in `target/generated-test-sources/akkaserveless/scala/com/example/shoppingcart/domain/`. The snippet `ShoppingCartSpec.scala` above shows all the available methods.
