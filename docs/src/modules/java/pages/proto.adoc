...
To use a multi-field key, add the `entity_key` notation to each field. For example, if the <<_domain_proto_file>> `AddLineItem` defines both the `user_id` and `product_id` as part of the key, do the same for the service message:
