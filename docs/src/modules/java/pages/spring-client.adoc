...
The Spring REST Client would enable Spring application to integrate with REST endpoints exposed via Akka Serverless Service.
...
Click Dependencies and select Spring Web and Spring Reactive Web.
...
Creating a Spring REST WebClient would allow Spring application to communicate with the REST Endpoints exposed via Akka Serverless Service.
...
The Service methods interact with endpoints defined in Akka Serverless Service.

The `CounterServiceImpl` shown in the following listing (from `akkaserverless-java-sdk/samples/java-valueentity-counter-spring-client/src/main/java/com/example/client/spring/rest/service/CounterServiceImpl.java`) uses the Spring REST WebClient to interact with the Akka Serverless Service and return the response received from called endpoint:
...
The `CounterController` shown in the following listing (from `akkaserverless-java-sdk/samples/java-valueentity-counter-spring-client/src/main/java/com/example/client/spring/rest/controller/CounterController.java`) uses the Spring service to get actual processing done:

[source,java,indent=0]
./src/main/java/com/example/client/spring/rest/controller/CounterController.java
----
include::example$java-valueentity-counter-spring-client/src/main/java/com/example/client/spring/rest/controller/CounterController.java[tag=getCounterEndpoint]
----
...
The `CounterClientApplication` shown in the following listing (from `akkaserverless-java-sdk/samples/java-valueentity-counter-spring-client/src/main/java/com/example/client/spring/rest/CounterClientApplication.java`) starts spring application:

[source,java,indent=0]
.src/main/java/com/example/client/spring/rest/CounterClientApplication.java
----
include::example$java-valueentity-counter-spring-client/src/main/java/com/example/client/spring/rest/CounterClientApplication.java[tag=counterApplication]
----
...
The Spring gRPC Client would enable Spring application to integrate with gRPC services exposed via Akka Serverless Service.
...
----
  <properties>
        <akkaserverless-sdk.version>0.10.6</akkaserverless-sdk.version>
...
- Get all the proto files from relevant akka serverless service and copy under `src/main/proto` folder.  Remove all the akka serverless annotations from the proto files. For instance `[(akkaserverless.field).entity_key = true]` and `akkaserverless.codegen` etc.
 For instance,  copying proto files from to `java-valueentity-counter/src/main/proto/com/example`.

- Run `mvn compile` to generate stubs.

=== Create a Spring gRPC Client

Creating a Spring gRPC would allow Spring application to communicate with the gRPC Services exposed via Akka Serverless Service.

cd into `akkaserverless-java-sdk/samples/java-valueentity-counter-spring-client`.

Now you can create a `GrpcClientConfig`. You need to configure the gRPC Client to use the Akka Serverless Service's host and port. The following listing (from `akkaserverless-java-sdk/samples/java-valueentity-counter-spring-client/src/main/java/com/example/client/spring/rest/config/GrpcClientConfig.java`) shows how to do so:
...
The Service methods interact with gRPC services defined in Akka Serverless Service.

The `GrpcClientService` shown in the following listing (from `akkaserverless-java-sdk/samples/java-valueentity-counter-spring-client/src/main/java/com/example/client/spring/rest/service/GrpcClientService.java`) uses the Spring REST WebClient to interact with the Akka Serverless Service and return the response received from called endpoint:
