...
Now you can create a `WebClient`. You need to configure the WebClient to use the Akka Serverless Service's host and port. The following listing shows how to do so:
...
The `CounterServiceImpl` shown in the following listing uses the Spring REST WebClient to interact with the Akka Serverless Service and return the response received from called endpoint:
...
- Get all the proto files from relevant akka serverless service and copy under `src/main/proto` folder.  Remove all the akka serverless annotations from the proto files. For instance `[(kalix.field).entity_key = true]` and `kalix.codegen` etc.
...
Now you can create a `GrpcClientConfig`. You need to configure the gRPC Client to use the Akka Serverless Service's host and port. The following listing shows how to do so:
...
The `GrpcClientService` shown in the following listing uses the Spring REST WebClient to interact with the Akka Serverless Service and return the response received from called endpoint:
