...
Each input message for an Entity that is input to a RPC command, *must* be associated with an Entity key, so that Akka Serverless can identify which entity the message is for. In the example, this includes `AddLineItem`, `RemoveLineItem`, and `GetShoppingCart` where the `user_id` is the `(kalix.field).entity_key`. Akka Serverless extracts the value of these fields in order to route messages to the right Entity. If more than one field is specified as an Entity key, the fields are concatenated together. Akka Serverless serializes Entity keys to strings.
...
It will often be necessary to access the generated entity key from inside the entities code. This can be done using the link:{attachmentsdir}/api/com/akkaserverless/javasdk/EntityContext.html#entityId()[`EntityContext.entityId` {tab-icon}, window="new"] method.
