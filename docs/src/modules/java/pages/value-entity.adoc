...
----
+
By default, the integration and unit tests are both invoked by `sbt test`. To invoke only the unit tests run `sbt -DonlyUnitTest test` or set up that value to `true` in the sbt console by `set onlyUnitTest := true`. Then run `sbt test`
