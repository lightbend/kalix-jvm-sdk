...
In this file we instruct the Akka Serverless Codegen which kind of component we want to create. The codegen will generated all stubs for your entity/service and corresponding tests, as well as an abstract class for your implementation to extend.
...
<12> `state` points to the protobuf message representing the Value Entity's state which is kept by Akka Serverless. ote, the package and name follow the definion in the domain.proto file.
...
<11> `state` points to the protobuf message representing the Value Entity's state which is kept by Akka Serverless. ote, the package and name follow the definion in the domain.proto file.
