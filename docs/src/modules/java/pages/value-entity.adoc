...
<3> The `CounterState` protobuf message is what Akka Serverless stores for this entity.
...
<2> The `CounterState` protobuf message is what Akka Serverless stores for this entity.
...
In this file we instruct the Akka Serverless Codegen which kind of component we want to create. The codegen will generate all stubs for your entity/service and corresponding tests, as well as an abstract class for your implementation to extend.
...
<12> `state` points to the protobuf message representing the Value Entity's state which is kept by Akka Serverless. Note, the package and name follow the definion in the domain.proto file.
...
<11> `state` points to the protobuf message representing the Value Entity's state which is kept by Akka Serverless. Note, the package and name follow the definion in the domain.proto file.
...
----
<1> Extends the generated `AbstractCounter`, which extends link:{attachmentsdir}/api/com/akkaserverless/scalasdk/valueentity/ValueEntity.html[`ValueEntity` {tab-icon}, window="new"].
<2> Defines the initial, empty, state that is used before any updates.
...
An example integration test class to start from is also generated for you. It uses [.group-java]#an `AkkaServerlessTestKitResource`# [.group-scala]#`KalixTestKit`# to start docker containers and interacts with the entity with an actual gRPC client.
