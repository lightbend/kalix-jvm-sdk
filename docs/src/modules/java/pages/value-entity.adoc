...
<2> Import the Akka Serverless protobuf annotations, or options.
<3> The protobuf option `(akkaserverless.file).value_entity` is specific to code-generation as provided by the Akka Serverless Maven plugin.
...
----
+
The unit tests can be run from sbt using `sbt test` or if you prefer from inside your IDE the same way you usually run tests.
