...
NOTE: After deleting the Value Entity it is possible to create it again, with the same entity ID.
...
./src/main/java/com/example/domain/Counter.java
----
include::java:example$java-valueentity-counter/src/main/java/com/example/domain/Counter.java[tag=delete]
----
...
./src/main/java/com/example/domain/Counter.scala
----
include::java:example$scala-valueentity-counter/src/main/scala/com/example/domain/Counter.scala[tag=delete]
----
<1> We delete the state by returning an `Effect` with `effects.deleteEntity()`.
