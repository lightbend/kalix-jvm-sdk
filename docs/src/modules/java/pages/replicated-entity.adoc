...
<1> Modify the Replicated Counter with `increment` or `decrement`.
<2> Trigger a replicated update by returning an `Effect` with `effects().update`.
...
<2> Update the Replicated Register value with `set`.
...
<2> Modify the elements of the Replicated Set with `add` or `remove`.
<3> Trigger a replicated update by returning an `Effect` with `effects().update`.

Replicated Set implements the `java.util.Set` interface and can be used directly as a collection to access the current elements:
...
<2> Modify the values of the Replicated Counter Map with `increment`, `decrement`, or `remove`.
<3> Trigger a replicated update by returning an `Effect` with `effects().update`.
...
<3> Modify the values of the Replicated Register Map with `setValue`, or `remove`.
<4> Trigger a replicated update by returning an `Effect` with `effects().update`.
...
<1> Create a domain object for the key.
<2> Create a domain object for the value.
<3> Modify the values of the Replicated Multi-Map with `put`, `putAll`, `remove`, or `removeAll`.
<4> Trigger a replicated update by returning an `Effect` with `effects().update`.
...
When implementing a Replicated Map entity, the replicated data for an entry can be retrieved using the `get` or `getOrCreate` methods on the current data object, and then triggering an update with the modified data object. Entries can be removed from the map using the `remove` method.
...
<2> Modify the Replicated Data value using its updating methods.
<3> Trigger a replicated update by returning an `Effect` with `effects().update`.
