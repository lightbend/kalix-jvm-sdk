...
The Akka Serverless Java/Scala SDK guides you through implementing components matching the protobuf APIs you design. This page describes prerequisites for Java development and basic requirements for a development project.

NOTE: Lightbend provides Tier 1 support for the Java and Scala SDKs. See https://developer.lightbend.com/docs/akka-serverless/faq/index.html#_what_languages_are_supported[an explanation of support tiers] for more information.

Your development project needs to include the Akka Serverless Java or Scala SDK and logic to start the gRPC server. You define your components in gRPC descriptors and use `protoc` to compile them. Finally, you implement business logic for service components.

To save the work of starting from scratch, the Java xref:java:quickstart-java-maven.adoc[code generation tool] creates a project via a Maven archetype, complete with descriptors and implementations. Or, you can start from one of our fully implemented https://developer.lightbend.com/docs/akka-serverless/quickstart/index.html[Quickstart applications].
...
Apache Maven:: For Java-based services, Akka Serverless relies on https://maven.apache.org/[Apache Maven {tab-icon}, window="new"] as build tool. The Akka Serverless Maven plugin creates initial Java code for your components and the required surrounding code.
...
The following examples show how to install the SDK to build your services with Maven. The code generation tools include an Akka Serverless xref:quickstart-java-maven.adoc[Maven archetype] that generates the recommended project structure, including a `.pom` file with the necessary references.
...
JSON formatted logging is enabled by default in the projects created by the xref:quickstart-java-maven.adoc[Akka Serverless Maven archetype]. It includes a dependency in `pom.xml` to `logback-json-classic` and a `logback.xml` file as shown here:
...
The Akka Serverless Maven plugin will create a main class for you and make sure all components get registered with the Akka Serverless server. The following code snippet shows an example that registers an Event Sourced Entity and starts the server:
...
* xref:quickstart-java-maven.adoc[]
* xref:proto.adoc[]
