...
[TIP]
 xref:tutorial:iotexample-learn.adoc[Build and deploy the IoT example] to see JSON formatted logging in action.
...
This page documents how to implement  Replicated Entities in Java. For information on what Replicated Entities are, please read how to xref:designing:choose-state-model.adoc[choose a state model] and the general xref:developing:replicatedentities.adoc[Replicated Entity] information first.
