...
Apart from the test kit, to facilitate the creation of unit tests, AkkaServerless generates test classes, ready to use, based on the Action defined in the .proto files. This is shown in the snippet below. 
...
[source,java]
----
include::example$java-eventsourced-counter/src/test/java/com/example/actions/CounterJournalToTopicActionTest.java[class] 
----
...
Calling an entity method through the TestKit gives us back an link:{attachmentsdir}/testkit/com/akkaserverless/javasdk/testkit/ActionResult.html[`ActionResult` {tab-icon}, window="new"]. This class has methods that we can use to assert our expectations, such as:

* `getReply` is the response of the method called in the test
* `getForwardServiceCall` returns the `ServiceCall` to which is fowarded the command the `Action` receives.
