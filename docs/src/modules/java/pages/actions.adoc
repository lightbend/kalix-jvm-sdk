...
<4> The protobuf option (kalix.codegen) is specific to code-generation as provided by the Akka Serverless Maven plugin. This annotation indicates to the code-generation that an Action must be generated.
...
<1> Any classes generated from this protobuf file will be in the `com.example.fibonacci` package.
<2> Import the Akka Serverless protobuf annotations or options.
<3> The protobuf option (kalix.codegen) is specific to code-generation as provided by the Akka Serverless sbt plugin. This annotation indicates to the code-generation that an Action must be generated.
...
*ActionResult*
+
Calling an action method through the test kit gives us back an link:{attachmentsdir}/testkit/com/akkaserverless/javasdk/testkit/ActionResult.html[`ActionResult` {tab-icon}, window="new"]. This class has methods that we can use to assert our expectations, such as:
+
...
*ActionResult*
+
Calling an action method through the test kit gives us back an link:{attachmentsdir}/scala-testkit-api/com/akkaserverless/scalasdk/testkit/ActionResult.html[`ActionResult` {tab-icon}, window="new"]. This class has methods that we can use to assert our expectations, such as:
+
