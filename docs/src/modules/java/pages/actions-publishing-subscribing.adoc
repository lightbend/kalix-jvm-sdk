...
+
[source,java,indent=0]
.src/main/java/com/example/actions/CounterJournalToTeffects().thenReply(Empty.getDefaultInstance())opicAction.java
----
...
----
<1> The `ignore` method is implemented as a catch-all. It matches `Any` input and replies with `effects().thenReply(Empty.getDefaultInstance())`.
...
----
<1> The `ignore` method is implemented as a catch-all. It matches `Any` input and replies with `effects().thenReply(Empty.defaultInstance)`.