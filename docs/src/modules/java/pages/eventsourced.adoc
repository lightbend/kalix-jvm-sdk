...
=== Unit Tests with autogenerated Testkit

We can test this Event Sourced Entity implementation with the SchoppingCartTestkit. This class is autogenerated by AkkaServerless when this project is compiled.

The following is a simplification of its contents but in essence, what we show in the next snippet is the signature of the available methods
...
----
<1> This constructor requires a `ShoppingCart` Entity so we can interact with this Entity through the `ShoppingCartTestKit`.
<2> This constructor allows us to set a state on the Entity, so we can start the test directly from the desired state instead of having to send all the commands that would produce such a state.
<3> This method returns the current state of the Entity. 
<4> This method returns the accumulated events from all the executed commands sent to the entity.
<5> This method calls to the method with the same name in the Event Source Entity passed in the constructor.


Once we call an RPC method on the entity this gives us back a `Result`. This class has the following methods that we can use to assert our expectations.

[source,java]
----
include::example$testkit/com/akkaserverless/javasdk/testkit/Result.java[tag=available-methods] 
----
<1> This `Reply` is the response of the RPC method called in the test 
<2> This `List<Object>` are the events generated upon the RPC method called in the test. This list can be inspected multiple times. This method is idempotent.
<3> This is the first of the events generated upon the RPC method called in the test. This event gets consumed once is inspected. This method is not idempotent.

Finally, the following snippet shows how to use the autogenerated TestKit to test our implementation. 
...
----
<1> creating the Testkit passing a clean instance of an Entity.
...
<4> checking the Result of the first call to addItem.
<5> checking the Result of all the calls to addItem.
...
It's worth noting that we have two sources of methods in this unit test. Those from the ShoppingCartTestKit itself and those from the Result. The former gives us the overall state of the entity. Its state and all that it had happened to it over the course of multiple RPC calls. While the latter gives us the effects produced for each singular call.  
