...
Commands can modify state by emitting events. A command handler can emit an event by taking in a link:{attachmentsdir}/api/com/akkaserverless/javasdk/eventsourcedentity/CommandContext.html[`CommandContext` {tab-icon}, window="new"] parameter, and invoking the link:{attachmentsdir}/api/com/akkaserverless/javasdk/eventsourcedentity/CommandContext.html#emit(java.lang.Object)[`emit` {tab-icon}, window="new"] method on it. When the command handler receives a request, it immediately emits the event. Upon which, Akka Serverless immediately sends the event to the journal and invokes the entity event handler. Immediately after, the entity state is updated to the new value and can be used to create the reply. This might occur before the event has been stored in the journal. This does not cause any concurrency issue because, Akka Serverless only process one command at a time per entity instance. The next command is not processed until an event from the previous command has been successfully been stored.
