...
This page documents how to implement Akka Serverless event sourced entities in Java. For information on what Akka Serverless event sourced entities are, please read the general xref:concepts:eventsourced.adoc[Event sourcing] documentation first.

An event sourced entity can be created by annotating it with the link:{attachmentsdir}/api/com/akkaserverless/javasdk/eventsourced/EventSourcedEntity.html[`@EventSourcedEntity`] annotation.

[source,java]
----
include::example$docs/user/eventsourcedentity/ShoppingCartEntity.java[tag=entity-class]
----

The `entityType` is used to namespace events in the journal. This is useful when you share the same database between multiple entities. It defaults to the simple name for the entity class (in this case, `ShoppingCartEntity`). It is good practice to explicitly define an `entityType`. This helps to decouple the journal data from the classnames in your code—the event naming in the journal remains coherent, even if the underlying entity classname may have changed.
...
Event sourced entities persist events and snapshots, and these need to be serialized when persisted. The most straight forward way to persist events and snapshots is to use protobufs. Akka Serverless will automatically detect if an emitted event is a protobuf, and serialize it as such. For other serialization options, including JSON, see xref:serialization.adoc[Serialization].
...
[source,java]
----
include::example$docs/user/eventsourcedentity/ShoppingCartEntity.java[tag=entity-state]
----

== Constructing

The context available for injection into the constructor is a link:{attachmentsdir}/api/com/akkaserverless/javasdk/eventsourced/EventSourcedEntityCreationContext.html[`EventSourcedEntityCreationContext`]. While you don't necessarily need to define a constructor, you can define one and have that context injected in. The constructor below shows how the `entityId` is injected:

[source,java]
----
include::example$docs/user/eventsourcedentity/ShoppingCartEntity.java[tag=constructing]
----

== Handling commands

Command handlers can be declared by annotating a method with link:{attachmentsdir}/api/com/akkaserverless/javasdk/eventsourced/CommandHandler.html[`@CommandHandler`]. They take a context class of type link:{attachmentsdir}/api/com/akkaserverless/javasdk/eventsourced/CommandContext.html[`CommandContext`].
...
[source,java]
----
include::example$docs/user/eventsourcedentity/ShoppingCartEntity.java[tag=get-cart]
----
...
Event handlers are declared by annotating a method with link:{attachmentsdir}/api/com/akkaserverless/javasdk/eventsourced/EventHandler.html[`@EventHandler`]. They take a context class of type link:{attachmentsdir}/api/com/akkaserverless/javasdk/eventsourced/EventContext.html[`EventContext`].
...
[source,java]
----
include::example$docs/user/eventsourcedentity/ShoppingCartEntity.java[tag=handle-snapshot]
----

== Registering the entity

Once you've created your entity, you can register it with the link:{attachmentsdir}/api/com/akkaserverless/javasdk/AkkaServerless.html[`AkkaServerless`] server, by invoking the `registerEventSourcedEntity` method. In addition to passing your entity class and service descriptor, you also need to pass any descriptors that you use for persisting events, for example, the `domain.proto` descriptor.

[source,java]
----
include::example$docs/user/eventsourcedentity/ShoppingCartEntity.java[tag=register]
----
