...
Event Sourced Entities persist events and snapshots, and these need to be serialized. Akka Serverless will automatically detect if an emitted event is a `protobuf`, and serialize it as such. For other serialization options, including JSON, see xref:serialization.adoc[].
