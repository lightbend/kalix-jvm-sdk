...
Event Sourced Entities persist events and snapshots, and these need to be serialized. Akka Serverless automatically detects if an emitted event is in `protobuf` format, and serializes it as such. For other serialization options, including JSON, see xref:serialization.adoc[].
...
Akka Serverless constructs instances of the Event Sourced Entity class on demand. The constructor below shows how the unique ID of the entity is injected and stored in link:{attachmentsdir}/api/com/akkaserverless/javasdk/EntityId.html[`entityId` {tab-icon}, window="new"].
...
When the command handler receives a request, the event is emitted and Akka Serverless immediately sends the event to the journal, and invokes the entity event handler.  Immediately after, the entity state is updated to the new value and can be used to create the reply. 

NOTE: An entity update event can occur before the event has been stored in the journal. This does not cause any concurrency issue because, Akka Serverless only process one command at a time per entity instance. The next command is not processed until an event from the previous command has been successfully been stored.
