...
The following snippet shows how the `ShoppingCartTestKit` is used to test the `ShoppingCart` implementation. AkkaServerless provides two main APIs for unit tests, the `ShoppingCartTestKit` and the `EventSourcedResult`. The former gives us the overall state of the entity. Its state and all the events over the course of all the RPC calls. While the latter only holds the effects produced for each individual RPC call to the Entity.
...
Calling an entity method through the TestKit gives us back an link:{attachmentsdir}/api/com/akkaserverless/javasdk/testkit/EventSourcedResult.html[`EventSourcedResult` {tab-icon}, window="new"]. This class has methods that we can use to assert our expectations, such as:
...
This class is generated by AkkaServerless when the project is compiled. The following is a simplification showing the signatures of its available methods.
...
----
<1> This constructor requires as input a `ShoppingCart` Entity constructor. Once the TestKit is created we can use it as a proxy to interact with the Entity it holds.
...
<5> This method calls to the method with the same name of the Entity created and contained by the `ShoppingCartTestKit`.
