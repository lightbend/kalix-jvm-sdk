...
Akka Serverless provides the link:{attachmentsdir}/api/com/akkaserverless/javasdk/valueentity/ValueEntity.html[`JsonSupport` {tab-icon}, window="new"] utility to serialize and deserialize JSON messages.
...
----
include::java:example$doc-snippets/src/main/proto/actions/json_api.proto[tag=service]
----
...
----
include::java:example$doc-snippets/src/main/java/com/example/json/MyServiceAction.java[tag=action]
----
