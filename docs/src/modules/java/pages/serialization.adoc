...
If the publishing service is also an Akka Serverless service, this is handled transparently for you as shown in xref:actions-publishing-subscribing.adoc[].

For messages consumed from or published to topics to interact with external services it can be a requirement to use another format than `protobuf`.
...
Java::
+
A `proto` definition of an Action that consumes cloud event messages can look like this:
+
[source,proto,indent=0]
.src/main/proto/com/example/topics_action.proto
----
...
----
<1> When consuming a cloud event containing a protobuf message the handler request must have the message type specified in the metadata
