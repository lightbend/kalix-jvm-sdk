...
To get started writing Akka Serverless services in Java, you need to install prerequisites.

== Prerequisites

The following are required to start writing Akka Serverless services in Java:
...
| *Java version*
| Akka Serverless Java support requires at least Java {minimum-java-version}, though we recommend using Java {recommended-java-version}, which has better support for running in containers. While it is possible to build a GraalVM native image for Akka Serverless Java user functions, at this stage Akka Serverless offers no specific assistance or configuration to aid in doing that.
...
| *Docker*
| Akka Serverless runs in Kubernetes using Docker. Hence you will need Docker to build a container that you can deploy to Kubernetes. Most popular build tools have plugins that assist in building Docker images.

| *Akka Serverless Java support library*
| See the instructions in <<Install the Akka Serverless library>>.

|===

== Install the Akka Serverless library

As mentioned, you can use the build tool of your choice. The following examples show how to install the Java Akka Serverless library for Gradle, Maven, and sbt.
...
[subs="attributes+"]
----
libraryDependencies += "com.akkaserverless" % "akkaserverless-java-sdk" % "{akkaserverless-java-sdk-version}"
----
...
Your main class will be responsible for creating the Akka Serverless gRPC server, registering the entities for it to serve, and starting it. To do this, you can use the link:{attachmentsdir}/api/com/akkaserverless/javasdk/AkkaServerless.html[`AkkaServerless`] server builder, for example:
...
| link:{attachmentsdir}/api/com/akkaserverless/javasdk/Context.html[`Context`]
|
...
| link:{attachmentsdir}/api/com/akkaserverless/javasdk/Metadata.html[`Metadata`]
|
| The metadata associated with the command.

| link:{attachmentsdir}/api/com/akkaserverless/javasdk/CloudEvent.html[`CloudEvent`]
|
