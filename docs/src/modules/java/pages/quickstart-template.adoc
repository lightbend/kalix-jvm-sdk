...
----
[.group-scala]#By default the integration and unit tests are both triggered with `sbt test`. To invoke only the unit tests run `sbt -DonlyUnitTest test` or set up that value to `true` in the sbt session by `set onlyUnitTest := true`. Then run `sbt test`#
