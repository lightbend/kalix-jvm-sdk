...
The Kalix plugin creates a unit test stub for the Entity. Use this stub as a starting point to test the logic in your implementation. [.group-java]#The Kalix Java SDK test kit supports both link:https://junit.org/junit4/[JUnit 4] and link:https://junit.org/junit5/[JUnit 5].#
...
For more details, see xref:java:value-entity.adoc#_testing_the_entity[Testing a Value Entity].
xref:java:event-sourced-entities.adoc#_testing_the_entity[Testing an Event Sourced Entity].
xref:java:actions.adoc#_testing_the_action[Testing an Action].

[.group-java]
The Maven plugin also provides you with an initial setup for integration tests based on the Kalix Java SDK test kit which leverages link:https://www.testcontainers.org/[TestContainers] and link:https://junit.org/junit4/[JUnit].
...
The first target, `deploy`, packages and publishes your Docker image to your repository and the second target, `kalix:deploy`, deploys the service with this image to Kalix. If `kalixProject` is set, that is the project where the service will be deployed. If it is not set, the service will be deployed in your currently selected project.
IMPORTANT: If you time stamp your image. For example, ` <dockerTag>${project.version}-${build.timestamp}</dockerTag>` you must always run both targets in one pass, i.e. `mvn deploy kalix:deploy`. You cannot run `mvn deploy` first and then `mvn kalix:deploy` because they have different timestamps and thus different `dockerTag`s. This makes it impossible to reference the image in the repository from the second target.
...
- Deploy your service following the instructions at xref:developing:development-process-java.adoc#_deploy_to_kalix[Development Process - Deploy to Kalix].
