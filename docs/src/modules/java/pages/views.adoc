...
<5> The `(kalix.method).eventing.in` annotation with `handle_deletes: true` and (required) `google.protobuf.Empty` as a parameter defines that this method is used for deleting the View. Kalix will automatically delete the View state, but you can also write your own delete handler when transform_updates` flag is `true`. If you don't provide such a method then, by default, your View state will not be deleted.
