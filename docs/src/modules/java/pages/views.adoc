...
NOTE: For index efficiency, only constant patterns with literal strings are supported, and the pattern must have a non-wildcard prefix or suffix. A pattern like `'%foo%'` is not supported.
