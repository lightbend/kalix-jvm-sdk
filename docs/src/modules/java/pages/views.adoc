...
<5> The second `(kalix.method).eventing.in` annotation with `handle_deletes: true` and (required) `google.protobuf.Empty` as a parameter defines that this method is used for handling Value Entity deletes. Kalix will automatically delete the View state, but you can also write your own delete handler when `transform_updates` flag is `true`. If you don't provide such method then, by default, your View state will not be deleted.
...
[source,proto,indent=0]
----
include::example$java-valueentity-customer-registry/src/main/proto/customer/view/customer_view.proto[tag=restructured]
----
...
[source,proto,indent=0]
----
include::example$scala-valueentity-customer-registry/src/main/proto/customer/view/customer_view.proto[tag=restructured]
----
<1> The `customer_id` column is projected into the `id` field in the result message.
<2> The customer `name` column is projected directly into the result message.
<3> A nested message is constructed with the address `street` (renamed to `address`), the address `city`, and the `email` columns, projected into `billing_details`.
