...
Fields in a Protobuf message that was not given a value is handled as [the default value](https://developers.google.com/protocol-buffers/docs/proto3#default) of the field data type.
 ...
 * make the field a part of a nested message and omit that whole nested message, for example `address.street` where there lack of an `address` message implies there is no `street` field.
...
Optional fields in query requests messages are handled like normal fields if they have a value, however a missing optional request parameters are seen as an invalid request and lead to a bad request response.
