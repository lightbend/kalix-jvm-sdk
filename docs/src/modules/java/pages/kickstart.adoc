...
* An xref:getting-started:projects.adoc#_create_a_project[Akka Serverless project]
* The xref:getting-started:set-up-development-env.adoc#_akka_serverless_cli[akkasls] CLI
* A configured registry in which to publish the service container image. Refer to xref:deploying:registries.adoc[] for more information on how to make your Docker registry available to Akka Serverless.
...
Akka Serverless uses gRPC Protocol Buffers language to describe the service interface and the entity domain model. The archetype generates a `CounterService` API implemented as a xref:reference:glossary.adoc#value_entity[Value Entity]. The entity descriptors include:
