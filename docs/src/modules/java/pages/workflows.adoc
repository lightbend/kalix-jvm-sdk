...
<2> Make sure to annotate such class with `@Type` and pass a unique name for this workflow type.
<3> Annotate such class with `@Key`  and pass the name of the key that will be used as the workflow unique identifier.
<4> Use Spring's RequestMapping annotation to define the route to your workflow.


NOTE: The Key `transferId` must match a path parameter (i.e. `transferId`) and such value needs to be unique per workflow. On the other hand, the Type `transfer` is common for all instances of this workflow but must be stable - cannot be changed after a production deploy - and unique across the different workflow types.
