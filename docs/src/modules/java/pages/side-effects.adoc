...
The side effects of an Action can be tested in isolation. To test the side effects of `DoubleCounterAction`, shown on the previous snippet, we can leverage link:{attachmentsdir}/testkit/com/akkaserverless/javasdk/testkit/ActionResult.html[`ActionResult` {tab-icon}, window="new"]. This class has the method `getSideEffects()` that returns the list of side effects added to the `Action`.
...
<2> retrieving the first side effect. There is only one in `DoubleConterAction.increase`. It's worth noting the side effects are `ServiceCall` objects that represent Akka Serverless RPC services. `ServiceCallDetails` is the representation of a `ServiceCall` on the Akka Serverless test kit framework.
...
<2> retrieving the first side effect. There is only one in `DoubleConterAction.increaseWithSideEffect` implementation. It's worth noting the side effects are `ServiceCall` objects that represent Akka Serverless RPC services. `ServiceCallDetails` is the representation of a `ServiceCall` on the Akka Serverless test kit framework.
<3> retrieving and asserting the name of the service
<4> retrieving and asserting the RPC's name of the service
<5> retrieving and asserting the RPC's input