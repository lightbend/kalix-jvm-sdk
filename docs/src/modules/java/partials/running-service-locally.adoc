...
The Akka Serverless proxy is required running local instances of a service. The proxy is packaged in a Docker container image available on the https://console.cloud.google.com/gcr/images/akkaserverless-public/GLOBAL/akkaserverless-proxy?project=akkaserverless-public&organizationId=401496590510&gcrImageListsize=30[Akka Serverless-Public container registry]. The example `docker compose` files is configured to leverage the proxy and run the application locally.

It also contains the configuration to start a local Google Pub/Sub emulator that the Akka Serverless proxy connects to when you make use of xref:java:topic-eventing.adoc[] in your service.
